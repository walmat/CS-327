...
#define range_lu_entry(range) { #range, RANGE_##range }
static const struct {
  const char *name;
  const object_range_t value;
} range_lookup[] = {
  range_lu_entry(ZERO),
  range_lu_entry(ONE),
  range_lu_entry(TWO),
  range_lu_entry(THREE),
  range_lu_entry(FOUR),
  range_lu_entry(FIVE),
  { 0, RANGE_ZERO }
};
...
static uint32_t parse_object_range(std::ifstream &f,
                            std::string *lookahead,
                            object_range_t *range)
{
  ...
  *range = RANGE_ZERO;
  ...
  for (i = 0; i < MAX_RANGE; i++) {
    if (*lookahead == range_lookup[i].name) {
      *range = range_lookup[i].value;
      break;
    }
  }
                                         ...
                                         std::vector<object_description> *v)
{
  ...
  bool read_name, read_desc, read_type, read_color,
    ...
    read_weight, read_speed, read_attr, read_val, read_range;
  std::string name, desc;
  uint32_t color;
  object_range_t range;
  object_type_t type;
  ...
  read_name = read_desc = read_type = read_color =
              read_hit = read_dam = read_dodge = read_def =
    read_weight = read_speed = read_attr = read_val = read_range = false;
  ...
  for (f >> *lookahead, count = 0;
    ...
    else if (*lookahead == "RANGE")    {
      if (read_range || parse_object_range(f, lookahead, &range)) {
        std::cerr << "Discovered at " << __FILE__ << ":" << __LINE__ << "\n"
                  << "Parse error in object value.\n"
                  << "Discarding object." << std::endl;
        return 1;
  ...
  o.set(name, desc, type, color, range, hit, dam,
...
std::ostream &object_description::print(std::ostream &o)
{
  ...
  for (i = 0; range_lookup[i].name; i++) {
    if (range == range_lookup[i].value) {
      o << range_lookup[i].name << std::endl;
      break;
    }
  }

  return o << hit << std::endl << damage << std::endl << range << std::endl
	   << dodge << std::endl << defence << std::endl << weight << std::endl
	   << speed << std::endl << attribute << std::endl << value << std::endl;
}
