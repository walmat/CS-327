...
First started off converting all the .c files to .cpp and fixing the casting errors. For this week's project I commented out the dungeon generation code in order to display the list of monsters that I wanted. I parsed data from a text file saved in the >>$HOME$/.rlg327/<< directory. I used these to output to the screen using cout. Other than that, not much else was done for this project.

**1.08** VERSION 1.3

First started by making an object class where I could store and pull the parsed data from last weeks assignment. Inside npc.cpp I made a few changes but ran into a Floating Point Exception which I couldn't figure out and eventually gave up on. I made it so the npc was also being stored from the parsed in data from last weeks assignment. This made the monsters be generated from the .txt file insstead of randomly generating them. Currently stuck on the floating point exception and haven't implemnted objects into the dungeon yet, but have the functions ready to do so inside object.cpp. Color would be as easy as adding in a couple lines of code, but nothing will show unless I figure out the FPE which isn't happening. Cool.

**1.09** VERSION 1.4

A bunch of functions were made in pc.cpp which are basically described by their name. pc_equip & pc_unequip, pc_drop, pc_expunge, etc. These all work as should. I added 3 new functions in io.cpp to handle the displaying: io_list_inventory_display(), io_list_equipment_display(), & io_list_item_description_display(). Each of their own handle calls to the proper key mentioned in the CHANGELOG. The biggest problem I ran into with these is that I had a hard time with stringstream getting the word to wrap after 41 characters but I finally figured that out as you can see in io_list_item_description().

A couple background things I mentioned in the CHANGELOG were that when the PC equips and unequips items it's speed and other attributes change, so that was a little tricky, but I managed to get that working by playing around inside pc.cpp and calling pc_update_speed() at the end of equip and unequip. Another thing was making sure the damage was properly updated by weapons so I just summed up the weapons damage and added it to the player damage in a function I believe named get_pc_dmg() or something similar.

Other than that, this week went a lot better than last week. Wow.

**1.10** VERSION 1.5 

This week I added ranged combat functionality and a couple items into the lore. It was fun and pretty much self explanatory. To access the ranged combat system, one must have a ranged item equipped and press the 'r' while the monster is in a 3x3 area of the PC. Weapons are a little unbalanced but it was fun messing around with those. Also added a little bit of an end game to the roguelike. The PC must endure 5 levels of dungeon before he/she technically wins. There is no score system though yet, but I'll probably add one in either today or tomorrow. 
