...
int run_this(int argc, char* argv[]){
  ...
  if (argc == 1){
    ...
    char* path;
    //num_monsters = (int) strtol(argv[3], (char **)NULL, 10);
    num_monsters = 10;
    char input[100];
...
/*
Generates a random room from room array. Attempts to place it at random (x,y) coordinates
...
void print_screen(){
  ...
  for (int i = 0; i < 21; i++){
    for (int j = 0; j < 80; j++){
      if (dungeon[i][j].occupant == NULL) {
	printf("%c", dungeon[i][j].type);
      } else {
	printf("%c", dungeon[i][j].occupant->current);
      }
    }
    printf("\n");
  }
