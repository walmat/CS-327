...
void char_receive_dmg(character* c, int32_t dmg)
{
  c->hp -= dmg;
  if (c->hp <= 0) character_die(c);
}
void char_swap(dungeon_t* d, character* c1, character* c2)
{
  pair_t p;

  p[dim_x] = c1->position[dim_x];
  p[dim_y] = c1->position[dim_y];

  c1->position[dim_x] = c2->position[dim_x];
  c1->position[dim_y] = c2->position[dim_y];

  c2->position[dim_x] = p[dim_x];
  c2->position[dim_y] = p[dim_y];

  d->charmap[c1->position[dim_y]][c1->position[dim_x]] = c1;
  d->charmap[c2->position[dim_y]][c2->position[dim_x]] = c2;
