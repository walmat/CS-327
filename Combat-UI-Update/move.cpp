...
void do_combat(dungeon_t *d, character *atk, character *def)
{
  ...
  if (atk == d->pc) {
    dmg = get_pc_dmg(d->pc);
    char_receive_dmg(def, dmg);
    io_queue_message("Atk: %d %d", d->pc->hp, def->hp);
    io_queue_message("dummy");
  ...
  //pc is defending
    ...
    dmg = atk->damage->roll();
    io_queue_message("Def: %d %d", d->pc->hp, atk->hp);
    io_queue_message("dummy");
    char_receive_dmg(d->pc, dmg);
  } else {
...
void move_character(dungeon_t *d, character *c, pair_t next)
{
  ...
  if (!d->charmap[next[dim_y]][next[dim_x]]) {
  ...
  if (c == d->pc) {
    if (objxy(character_get_x(c), character_get_y(c))){
      pc_pickup_obj(c, objxy(character_get_x(c), character_get_y(c)));
      objxy(character_get_x(c), character_get_y(c)) = NULL;
    }
